...
이반적으로 재정의 전의 멤버 함수가 호출됨.
- C++ 컴파일러는 포인터를 이용한 연산의 가능성 여부를 판단할때,
포인터의 자료형을 기준으로 판단하지, 실제 카리키는 객체의 자료형을
기준으로 판단하지 않는다.
...
소멸자만 virtual로 선언하며, 이를 상속하는 파생 클래스의 소멸자들도 가상 소멸자로 선언이 된다.

가상함수를 파생 클래스에서 재정의하는 것을 오버라이딩 이라고 함
*/